...
export function registerActionHandlers(api: PluginAPI): void {
  const handleAction = async (
    ...
    switch (action) {
      ...
      // -------------------------------------------------------------- //
      case 'refresh-status': {
        const { syncRuntime } = await import('./index.js');
        return syncRuntime(api, { reason: 'manual-refresh', notify: false, recordHistory: true });
      }

      case 'refresh-dashboard': {
        const { refreshDashboardSnapshot } = await import('./index.js');
      ...
      case 'load-recent-events': {
        const { loadRecentEvents } = await import('./index.js');
      ...
      case 'notification-mark-read': {
        const { setNotificationReadState } = await import('./index.js');
        return setNotificationReadState(api, cleanText(data?.id as string), true);
      }

      case 'notification-mark-all-read': {
        const { markAllNotificationsRead } = await import('./index.js');
        return markAllNotificationsRead(api);
      }

      case 'notification-clear': {
        const { clearNotifications } = await import('./index.js');
      ...
      case 'execute-command': {
        const { executeDaemonCommand } = await import('./index.js');
        return executeDaemonCommand(api, cleanText(data?.input as string));
      }

      case 'create-subagent': {
        const { createDaemonSubAgent } = await import('./index.js');
      ...
      // -------------------------------------------------------------- //
      case 'daemon-call': {
        const { daemonAction } = await import('./index.js');
