...
export function LegionSettings({ pluginState, pluginConfig, setPluginConfig, onAction }: any): any {
  ...
  const renderActiveSection = () => {
    ...
    if (activeSection === 'behavior') {
      return <BehaviorTab draft={draft} setDraft={setDraft} />;
    }
    if (activeSection === 'threads') {
      return <ThreadsTab draft={draft} setDraft={setDraft} saving={saving} onSave={saveDraft} />;
    }

    // Diagnostics tabs
    if (activeSection === 'doctor') {
      return <DoctorTab pluginState={pluginState} />;
    }
    if (activeSection === 'runtime') {
      return <RuntimeTab pluginState={pluginState} draft={draft} />;
    }

    // Daemon Management tabs
    const DaemonTab = daemonTabMap[activeSection];
    if (DaemonTab) {
      return <DaemonTab onAction={onAction} />;
    }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <h2 className="text-base font-semibold">Legion</h2>
          <Badge status={pluginState?.status} />
        </div>
        <div className="flex flex-wrap gap-1.5">
          <ActionButton label={saving ? 'Saving...' : 'Save'} onClick={saveDraft} disabled={saving} />
          <ActionButton label={working ? '...' : 'Refresh'} onClick={() => runAction('refresh-status')} disabled={working} variant="secondary" />
          <ActionButton label="Doctor" onClick={() => runAction('run-doctor')} disabled={working} variant="secondary" />
        </div>
      </div>
      {/* Status note */}
      {note && (
        <div className="rounded-xl border border-border/60 bg-background/45 px-3 py-2 text-xs">{note}</div>
      )}
      {/* Tab strip -- compact horizontal pills grouped with labels */}
      <div className="space-y-2">
        {groups.map((group) => (
          <div key={group}>
            <div className="mb-1 text-[10px] font-medium uppercase tracking-wider text-muted-foreground/70">{group}</div>
