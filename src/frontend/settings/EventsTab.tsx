...
export function EventsTab({ onAction }: { onAction: any }) {
  ...
  return (
    <Section
      ...
      actions={[
        <ActionButton key="r" label={loading ? 'Refreshing...' : 'Refresh'} onClick={refresh} disabled={loading} variant="secondary" />,
      ]}
    >
      {loading && events.length === 0 ? (
        <p className="text-sm text-muted-foreground">Loading...</p>
      ) : error ? (
        <div className="rounded-2xl border border-red-500/30 bg-red-500/5 px-4 py-3 text-sm text-red-600 dark:text-red-300">
          {error}
          <button className="ml-3 text-xs underline" onClick={refresh}>Retry</button>
        </div>
      ) : (
        <div className="grid gap-3">
          ...
          {/* Event feed */}
          {events.length === 0 ? (
            <EmptyState title="No events" body="No recent events from the daemon." />
          ) : (
            <div
              ref={feedRef}
            ...
            >
              {events.map((evt: any, i: number) => {
                ...
                return (
                  <div key={key || i} className="rounded-xl border border-border/50 bg-background/45 px-3 py-2">
                    <div
                    ...
                    >
                      ...
                      {evt.source && (
                        <span className="text-xs text-muted-foreground truncate">{evt.source}</span>
                      )}
                      <span className="ml-auto text-[10px] text-muted-foreground/60">{isExpanded ? '[-]' : '[+]'}</span>
                    </div>
                    {isExpanded && (
                      <div className="mt-2">
                        <JsonBox value={evt} emptyLabel="No payload." />
                      </div>
                    )}
                  </div>
                );
