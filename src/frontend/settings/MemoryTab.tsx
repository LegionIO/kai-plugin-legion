...
function typeBadge(type: string) {
  ...
  return (
    <span className={cx('inline-flex rounded-full px-2.5 py-1 text-[11px] font-semibold capitalize', colors)}>
      {type || 'unknown'}
    </span>
  );
}
...
export function MemoryTab({ onAction }: { onAction: any }) {
  ...
  return (
    <Section
      title="Memory"
      subtitle="Memory entries and statistics"
      ...
      {loading ? (
        ...
        <div className="grid gap-4">
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
            <StatCard label="Total" value={fmtNumber(stats?.total ?? stats?.count ?? items.length)} />
            <StatCard label="Working" value={fmtNumber(countByType('working'))} subvalue="Short-term context" />
            <StatCard label="Observational" value={fmtNumber(countByType('observational'))} subvalue="Noted patterns" />
            <StatCard label="Semantic" value={fmtNumber(countByType('semantic'))} subvalue="Embedded knowledge" />
            {stats?.embeddingModel && <StatCard label="Embedding" value={stats.embeddingModel} />}
          </div>

          {/* Filter tabs */}
          <SegmentTabs tabs={FILTER_TABS} active={filter} onChange={setFilter} />
          ...
          <div className="text-xs text-muted-foreground">
            {filtered.length} of {items.length} entries{debouncedSearch ? ` matching "${debouncedSearch}"` : ''}
          </div>

          {/* Entries list */}
          {filtered.length === 0 ? (
            <EmptyState title="No entries" body={filter !== 'all' || debouncedSearch ? 'No entries match the current filter.' : 'Memory store is empty.'} />
          ) : (
                  ...
                  <div key={id} className="rounded-2xl border border-border/60 bg-background/45 px-4 py-3">
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="truncate text-sm font-medium">{m.content || m.key || `Entry ${i + 1}`}</div>
                        <div className="flex flex-wrap items-center gap-2 mt-0.5">
                          {m.score != null && <span className="text-[11px] text-muted-foreground">score: {m.score}</span>}
                          {m.source && <span className="text-[11px] text-muted-foreground">source: {m.source}</span>}
                          {m.tags?.length > 0 && <span className="text-[11px] text-muted-foreground">{m.tags.join(', ')}</span>}
                          {m.createdAt && <span className="text-[11px] text-muted-foreground">{fmtAgo(m.createdAt)}</span>}
                        </div>
                      </div>
                      <span className="text-[10px] text-muted-foreground/60 shrink-0">{isExpanded ? '[-]' : '[+]'}</span>
                    </div>
                    {/* Expanded detail */}
                    {isExpanded && (
                        ...
                        {isEditing ? (
                          <div className="grid gap-2">
                            ...
                            <div className="flex gap-2">
                              <ActionButton label="Save" onClick={() => saveEdit(id)} />
                          ...
                          <div className="grid gap-2">
                            {/* Full content */}
                            {m.content && <div className="rounded-xl border border-border/40 bg-background/30 px-3 py-2 text-sm whitespace-pre-wrap">{m.content}</div>}
                            {/* Timestamps */}
                            <div className="flex flex-wrap gap-4 text-[11px] text-muted-foreground">
                              {m.createdAt && <span>Created: {fmtTime(m.createdAt)}</span>}
                              {m.updatedAt && <span>Updated: {fmtTime(m.updatedAt)}</span>}
                            </div>
                            {/* Full metadata as JSON */}
                            <JsonBox value={m} emptyLabel="No metadata." />
                            {/* Action buttons */}
                            <div className="flex gap-2">
                              <ActionButton label="Edit" onClick={() => startEdit(m)} variant="secondary" />
                              <ActionButton label="Delete" onClick={() => deleteEntry(id)} variant="danger" />
                            </div>
                          </div>
                        )}
                      </div>
