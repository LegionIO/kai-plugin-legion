        ...
        <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="grid gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              <ActionButton label={running ? 'Running...' : 'Run Command'} onClick={runCommand} disabled={running || !input.trim()} />
              <ActionButton label="Create Workspace Thread" onClick={() => onAction?.('create-thread', { open: true })} variant="secondary" />
              <ActionButton label="Open Proactive Thread" onClick={() => onAction?.('open-proactive-thread')} variant="secondary" />
            </div>
          </div>
          <div className="rounded-2xl border border-border/60 bg-background/45 p-4">
            <div className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Suggestions</div>
            {filtered.length === 0 ? (
              <p className="mt-3 text-sm text-muted-foreground">No daemon capabilities available yet.</p>
            ) : (
              <div className="mt-3 space-y-2">
                {filtered.map((capability: any, index: number) => (
                  <button
                  ...
                  >
                    <div className="text-sm font-medium">{capability?.name || 'Capability'}</div>
        ...
        <div className="grid gap-4 lg:grid-cols-[0.65fr_0.35fr]">
          <div className="grid gap-4">
            <div className="grid gap-4 md:grid-cols-[1fr_140px]">
              <Field label="Path" value={apiPath} onChange={setApiPath} placeholder="/api/settings" />
              <Field label="Method" value={apiMethod} onChange={(value: string) => setApiMethod(value.toUpperCase())} placeholder="GET" />
            </div>
            <TextAreaField label="Query JSON" value={apiQuery} onChange={setApiQuery} placeholder='{"count":"25"}' rows={4} />
            <TextAreaField label="Body JSON" value={apiBody} onChange={setApiBody} placeholder='{"key":"value"}' rows={6} />
            <Toggle label="Expect Text Response" description='Enable this for endpoints like `/api/metrics` that return plain text instead of JSON.' checked={apiExpectText} onChange={setApiExpectText} />
            <div className="flex flex-wrap gap-2">
              <ActionButton label={apiBusy ? 'Sending...' : 'Send Request'} onClick={runApiRequest} disabled={apiBusy || !apiPath.trim()} />
              <ActionButton label="Settings" onClick={() => applyPreset('/api/settings')} variant="secondary" />
              <ActionButton label="Schedules" onClick={() => applyPreset('/api/schedules')} variant="secondary" />
              <ActionButton label="Triggers" onClick={() => applyPreset('/api/triggers')} variant="secondary" />
              <ActionButton label="Memory Stats" onClick={() => applyPreset('/api/memory/stats')} variant="secondary" />
              <ActionButton label="Metrics" onClick={() => applyPreset('/api/metrics', 'GET', '{}', '{}', true)} variant="secondary" />
            </div>
          </div>
          <div className="rounded-2xl border border-border/60 bg-background/45 p-4">
            <div className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Common Surfaces</div>
            <div className="mt-3 space-y-2 text-sm text-muted-foreground">
              {['catalog', 'extensions', 'tasks', 'workers', 'schedules', 'audit', 'transport', 'prompts', 'webhooks', 'tenants', 'capacity', 'governance', 'rbac', 'nodes', 'memory', 'marketplace', 'github', 'gaia', 'metering', 'mesh', 'absorbers', 'structural_index', 'tool_audit', 'state_diff', 'sessions/search', 'triggers', 'llm/token_budget', 'llm/providers', 'llm/provider_layer', 'llm/context_curation/status'].map((label) => (
                <div key={label} className="rounded-xl border border-border/50 bg-card/50 px-3 py-2">{`/api/${label}`}</div>
              ))}
            </div>
          </div>
        </div>
        <div>
          <JsonBox value={apiResult} emptyLabel="Send a daemon request to inspect the raw response here." />
        </div>
      </Section>
    </div>
  );
